...
if(e.ctrlKey||e.altKey||e.metaKey||k<32){return}else if(k){if(pos.end-pos.begin!==0){clearBuffer(pos.begin,pos.end);shiftL(pos.begin,pos.end-1)}
...
input.one("unmask",function(){input.unbind(".mask").removeData($.mask.dataName)}).bind("focus.mask",function(){clearTimeout(caretTimeoutId);var pos;focusText=input.val();pos=checkVal();caretTimeoutId=setTimeout(function(){writeBuffer();if(pos==mask.length){input.caret(0,pos)}else{input.caret(pos)}},10)}).bind("blur.mask",function(){checkVal();if(input.val()!=focusText)
